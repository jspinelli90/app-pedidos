...
async function generatePricePdf(input) {
  ...
  function heading() {
    ...
    const headerHeight = Math.max(94, pdf.heightOfString(data.title, { width: width - 92 }) + 48);
    pdf.rect(0, 0, pdf.page.width, headerHeight).fill("#080808");
    pdf.rect(0, headerHeight, pdf.page.width, 3).fill("#b21f24");
    pdf.image(path.join(__dirname, "../public/san-cayetano-logo-blanco.png"), 42, 14, { fit: [78, 66] });
    pdf.fillColor("white").font("Helvetica-Bold").fontSize(15).text(data.title, 134, 24, { width: width - 92 });
    y = headerHeight + 20;
    pdf.rect(42, y, width, 26).fill("#e8eeeb");
    pdf.fillColor("#173c35").font("Helvetica-Bold").fontSize(10).text("PRODUCTO", 52, y + 8).text("PRECIO ($)", 426, y + 8, { width: width - 394, align: "right" });
    y += 32;
  }
  heading();
  data.rows.forEach((row, index) => {
    pdf.font("Helvetica").fontSize(10);
    const height = Math.max(28, pdf.heightOfString(row.name, { width: 350 }) + 14);
    if (y + height > 762) { pdf.addPage(); heading(); }
    if (index % 2 === 0) pdf.rect(42, y, width, height).fill("#f4f7f5");
    pdf.fillColor("#172b25").font("Helvetica").fontSize(10).text(row.name, 52, y + 7, { width: 350 });
    pdf.font("Helvetica-Bold").text(row.price === null ? "Consultar" : row.price.toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }), 412, y + 7, { width: width - 380, align: "right" });
    y += height;
  });
  if (data.notes) {
    pdf.font("Helvetica").fontSize(9);
    const height = pdf.heightOfString(data.notes, { width });
    if (y + height + 26 > 770) { pdf.addPage(); heading(); }
    pdf.fillColor("#4b6159").font("Helvetica").fontSize(9).text(data.notes, 42, y + 16, { width });
  }
  const range = pdf.bufferedPageRange();
  for (let page = 0; page < range.count; page++) {
    pdf.switchToPage(page);
    pdf.fillColor("#60746a").font("Helvetica").fontSize(8).text(`San Cayetano Carnes  |  ${page + 1} / ${range.count}`, 42, 780, { width, align: "center", lineBreak: false });
  }
  pdf.end();
  return complete;
}
