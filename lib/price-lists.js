...
function validatePriceList(input) {
  ...
  const rows = input.rows.map((row, index) => {
    ...
    if (row.id !== undefined && (typeof row.id !== "string" || !/^[a-zA-Z0-9_-]{1,80}$/.test(row.id))) throw invalid(`Identificador inválido en la fila ${index + 1}.`);
    if (row.unit !== undefined && !["", "kg", "unit", "box"].includes(row.unit)) throw invalid(`Unidad inválida en la fila ${index + 1}.`);
    return { ...(row.id ? { id: row.id } : {}), ...(row.unit !== undefined ? { unit: row.unit } : {}), name: row.name.trim(), price: row.price === null ? null : Math.round((row.price + Number.EPSILON) * 100) / 100 };
  });
  const ids = rows.filter(row => row.id).map(row => row.id);
  if (new Set(ids).size !== ids.length) throw invalid("Hay identificadores de productos repetidos.");
